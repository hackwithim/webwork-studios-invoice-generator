...
export default function AIAgentPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "Hello! I am your CRM AI Assistant. You can ask me questions about your clients and leads, or ask me to dispatch an Email or WhatsApp message for you. How can I help today?"
    }
  ]);
  ...
  useEffect(() => {
    setIsMounted(true);
    const savedHistory = localStorage.getItem("crm_ai_chat_history");
    if (savedHistory) {
      try {
  ...
  useEffect(() => {
    if (isMounted) {
      localStorage.setItem("crm_ai_chat_history", JSON.stringify(messages));
    }
  }, [messages, isMounted]);
  ...
  const handleClearHistory = () => {
    ...
    localStorage.removeItem("crm_ai_chat_history");
  };
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-none">
              <select
                ...
                className="appearance-none w-full sm:w-56 bg-white border border-slate-200 text-slate-700 text-sm font-medium rounded-xl pl-4 pr-10 py-2 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent cursor-pointer shadow-sm"
              >
                {MODELS.map((m) => (
                ...
                <div className={`flex flex-col max-w-[80%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
                  {msg.isDispatchSuccess ? (
                    ...
                    <div className={`px-5 py-3 shadow-sm text-sm inline-block ${
                      msg.role === "user" 
                        ? "bg-accent text-white rounded-3xl rounded-tr-sm" 
        ...
        <div className="p-4 sm:p-6 border-t border-slate-200 bg-white z-10">
          <div className="flex items-end gap-3 max-w-4xl mx-auto">
            <textarea
              ...
              className="flex-1 px-5 py-4 rounded-2xl border border-slate-200 focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-all resize-none text-sm min-h-[56px] max-h-32 bg-slate-50 hover:bg-white focus:bg-white shadow-sm"
              disabled={isLoading}
            />
            <button
              onClick={handleSend}
              disabled={isLoading || !prompt.trim()}
              className="shrink-0 mb-1 w-12 h-12 flex items-center justify-center bg-accent hover:bg-blue-600 text-white rounded-xl transition-all shadow-sm disabled:opacity-50 disabled:pointer-events-none"
            >
