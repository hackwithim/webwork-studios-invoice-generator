...
import { createQuotation, getNextQuotationNumber, getQuotationById, updateQuotation } from "@/actions/quotation";
...
function CreateQuotationContent() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      getQuotationById(editId).then(quotation => {
        if (quotation) {
          setValue("quotationNumber", quotation.quotationNumber);
          setValue("clientId", quotation.clientId);
          setValue("date", new Date(quotation.invoiceDate).toISOString().split('T')[0]);
          setValue("validityDate", new Date(quotation.validityDate).toISOString().split('T')[0]);
          setValue("notes", quotation.notes || "");
          setValue("terms", quotation.terms || "");
          setValue("isTaxable", quotation.taxRate > 0);
          setValue("taxRate", quotation.taxRate || 18);
      ...
      getNextQuotationNumber().then(num => setValue("quotationNumber", num)).catch(console.error);
    }
  }, [editId]);
  ...
  const { register, control, watch, handleSubmit, setValue, formState: { errors } } = useForm<FormValues>({
    defaultValues: {
      ...
      terms: "This quotation is valid for 14 days.",
      isTaxable: true,
      taxRate: 18,
    }
  });
  ...
  const onSubmit = async (data: FormValues, status: "DRAFT" | "PUBLISHED") => {
    ...
    try {
      if (editId) {
        await updateQuotation(editId, { ...data, subtotal, gst, total, status, taxRate: watchIsTaxable ? watchTaxRate : 0 });
      } else {
        await createQuotation({ ...data, subtotal, gst, total, status, taxRate: watchIsTaxable ? watchTaxRate : 0 });
      }
      router.push("/dashboard/quotations");
    } catch (error) {
      console.error("Failed to save quotation", error);
      alert("Failed to save quotation. Please ensure you have created a client first and selected one.");
    } finally {
    ...
    <div className="max-w-7xl mx-auto pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div className="flex items-center gap-3">
          <Link href="/dashboard/quotations" className="p-2 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-primary transition-colors bg-white shadow-sm">
            <ArrowLeft size={18} />
          </Link>
          <div>
            ...
            <p className="text-sm text-slate-500">
              {editId ? "Update your existing quotation." : "Draft a new quotation for your client."}
            </p>
          </div>
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  {...register("quotationNumber", { required: "Required" })}
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  type="date"
                  {...register("validityDate", { required: "Required" })}
                  className="w-full h-11 px-4 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all text-slate-700"
                />
              </div>
            </div>
              ...
              <div className="space-y-1.5 relative">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <AiAssistButton 
                    ...
                    context="Creating a quotation. Need professional notes to the client outlining the proposal."
                    placeholder="e.g. Write a professional opening note"
                  />
                </div>
                <textarea 
                  {...register("notes")}
              ...
              <div className="space-y-1.5 relative">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <AiAssistButton 
                    ...
                    context="Creating a quotation. Need professional terms and conditions regarding validity period."
                    placeholder="e.g. Write standard quotation terms"
                  />
                </div>
                <textarea 
                  {...register("terms")}
